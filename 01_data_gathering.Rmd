...
(all_streamflow_data <- all_stream_data %>% 
...
```{r get-stream-data}
# get stream data
(all_stream_data_dv <- readNWISdv(siteNumbers = all_stream_data$site_no,
           parameterCd = "00060",
           startDate = start_date) %>% renameNWISColumns())
```
...
all_stream_data_dv %>% 
  group_by(site_no) %>% 
  summarise(n = n(), min_date = min(Date), max_date = max(Date)) %>% 
  left_join(all_stream_data %>% select(site_no, station_nm) %>% distinct(site_no,station_nm)) %>% 
  select(site_no, station_nm, everything())
```
